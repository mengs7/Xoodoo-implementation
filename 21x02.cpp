...
int verticalsymmetry(int* p[2]) // draw a vertical line cutting the 2d array into 2 parts and return 1 if the 2 parts are symmetric
{
    if (x<2) return 0;
    else {
        for (int i=0; i<z; i++)
...
int horizontalsymmetry(int* p[2])
{
    if (z<2) return 0;
    else {
        for (int j=0; j<x; j++)
...
void printstatebinary(int* planea[2], int* planeb[2], int* planec[2])
{
    for(int i=0;i<z;i++)
    {
        for(int j=0;j<x;j++)
...
void printstatedecimal(int* out[2])
{
    for(int i=0;i<z;i++)
    {
        for(int j=0;j<x;j++)
...
void insertintostate(int q, int* planea[2], int* planeb[2], int* planec[2])
{
...
void planetostate(int* planea[2], int* planeb[2], int* planec[2], int* out[2])
{
    for (int i=0; i<z; i++)
    {
        for (int j=0; j<x; j++)
...
string statetobinary(int* planea[2], int* planeb[2], int* planec[2])
{
    string s = "";
    for (int i=0; i<z; i++)
    {
...
void singlepositionshift(int* planea[2], int* planeb[2], int* planec[2])
{
    int q=0;
    int *a = new int[x*z];
    int *b = new int[x*z];
...
int checkplane(int* planea[2], int* planeb[2])
{
    for (int i=0; i<z; i++)
    {
        for (int j=0; j<x; j++)
...
void equateplane(int* planea[2], int* temp[2])
{
    for (int i=0; i<z; i++)
    {
        for (int j=0; j<x; j++)
...
void shiftplane(int xshift, int zshift, int* planea[2], int* temp[2])
{
    for (int i=0; i<z; i++)
    {
        for (int j=0; j<x; j++)
...
void xorplane(int* planea[2], int* planeb[2], int* temp[2])
{
    for (int i=0; i<z; i++)
    {
        for (int j=0; j<x; j++)
...
void complimentplane(int* planea[2], int* temp[2])
{
    for (int i=0; i<z; i++)
    {
        for (int j=0; j<x; j++)
...
void andplane(int* planea[2], int* planeb[2], int* temp[2])
{
    for (int i=0; i<z; i++)
    {
        for (int j=0; j<x; j++)
...
void mixinglayer(int* planea[2], int* planeb[2], int* planec[2], int* temp[2])
{
...
void rhowest(int* planea[2], int* planeb[2], int* planec[2], int* temp[2])
{
...
void rhoeast(int* planea[2], int* planeb[2], int* planec[2], int* temp[2])
{
    //cout<<"rhoeast"<<endl;
...
void nonlinearlayer(int* planea[2], int* planeb[2], int* planec[2], int* temp[2])
{
...
void roundfunction(int* planea[2], int* planeb[2], int* planec[2], int* temp[2])
{
...
int statecyclecheck(int* out[2], int* check[2])
{
    int *o = new int[x*z];
    int *c = new int[x*z];
    int q = 0;
...
void shiftcheck(int xshift, int zshift, int* planea[2], int* planeb[2], int* planec[2], int* newplanea[2], int* newplaneb[2], int* newplanec[2])
{
...
int whileconditioncheck(int* planea[2], int* newplanea[2])
{
    int** extra = new int* [z];
    for (int i=0; i<z; i++)
        extra[i] = new int[x];
    ...
    for (int shift = 0; shift < z; shift++)
    {
        ...
        if (checkplane(newplanea, extra)) {if (shift==0) return 4; else return 2;}
        shiftplane(shift,1,planea,extra);
        if (checkplane(newplanea, extra)) return 2;
    }
    return -1;
}
...
int main()
{
    ...
    for (int q=0; q<total_inputs; q++)
    {
        ...
        else
        {
            ...
            cout<<endl<<"Input in decimal: "<<q<<endl;
            ...
            cout<<endl<<"First state in cycle: "<<endl;
            printstatedecimal(state);
            cout<<"Last state in cycle: "<<endl;
            cout<<"cycle length: "<<count<<endl;
            cout<<"number of cycles in this superstate set: "<<offset<<endl;
    ...
    int total = 0;
    cout<<endl<<"CYCLE RESULTS: "<<endl<<endl;
