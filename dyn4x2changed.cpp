...
int verticalsymmetry(int* p[4]) // draw a vertical line cutting the 2d array into 2 parts and return 1 if the 2 parts are symmetric
{
    if (x<2) return 0;
    else {
        for (int i=0; i<z; i++)
...
int horizontalsymmetry(int* p[4])
{
    if (z<2) return 0;
    else {
        for (int j=0; j<x; j++)
...
void printstatebinary(int* planea[4], int* planeb[4], int* planec[4])
{
    for(int i=0;i<z;i++)
    {
        for(int j=0;j<x;j++)
...
void printstatedecimal(int* out[4])
{
    for(int i=0;i<z;i++)
    {
        for(int j=0;j<x;j++)
...
void insertintostate(int q, int* planea[4], int* planeb[4], int* planec[4])
{
...
void planetostate(int* planea[4], int* planeb[4], int* planec[4], int* out[4])
{
    for (int i=0; i<z; i++)
    {
        for (int j=0; j<x; j++)
...
string statetobinary(int* planea[4], int* planeb[4], int* planec[4])
{
    string s = "";
    for (int i=0; i<z; i++)
    {
...
void singlepositionshift(int* planea[4], int* planeb[4], int* planec[4])
{
    int q=0;
    int *a = new int[x*z];
    int *b = new int[x*z];
...
int checkplane(int* planea[4], int* planeb[4])
{
    for (int i=0; i<z; i++)
    {
        for (int j=0; j<x; j++)
...
void equateplane(int* planea[4], int* temp[4])
{
    for (int i=0; i<z; i++)
    {
        for (int j=0; j<x; j++)
...
void shiftplane(int xshift, int zshift, int* planea[4], int* temp[4])
{
    for (int i=0; i<z; i++)
    {
        for (int j=0; j<x; j++)
...
void xorplane(int* planea[4], int* planeb[4], int* temp[4])
{
    for (int i=0; i<z; i++)
    {
        for (int j=0; j<x; j++)
...
void complimentplane(int* planea[4], int* temp[4])
{
    for (int i=0; i<z; i++)
    {
        for (int j=0; j<x; j++)
...
void andplane(int* planea[4], int* planeb[4], int* temp[4])
{
    for (int i=0; i<z; i++)
    {
        for (int j=0; j<x; j++)
...
void mixinglayer(int* planea[4], int* planeb[4], int* planec[4], int* temp[4])
{
    ...
    shiftplane(1,1,P,temp); //shifts P, state_parsed in temp
    shiftplane(1,0,E,P);//shifts E, puts new value into P
...
void rhowest(int* planea[4], int* planeb[4], int* planec[4], int* temp[4])
{
    shiftplane(1,0,planeb,temp);
    equateplane(temp,planeb);
    
    shiftplane(0,1,planec,temp);
    equateplane(temp,planec);
}

void rhoeast(int* planea[4], int* planeb[4], int* planec[4], int* temp[4])
{
    //cout<<"rhoeast"<<endl;
    ...
    shiftplane(2,0,planec,temp);
    equateplane(temp,planec);
    //printstatebinary(planea,planeb,planec);
}

void nonlinearlayer(int* planea[4], int* planeb[4], int* planec[4], int* temp[4])
{
...
void roundfunction(int* planea[4], int* planeb[4], int* planec[4], int* temp[4])
{
...
int statecyclecheck(int* out[4], int* check[4])
{
    int *o = new int[x*z];
    int *c = new int[x*z];
    int q = 0;
...
void shiftcheck(int xshift, int zshift, int* planea[4], int* planeb[4], int* planec[4], int* newplanea[4], int* newplaneb[4], int* newplanec[4])
{
...
int whileconditioncheck(int* planea[4], int* newplanea[4])
{
    int** extra = new int* [z];
    for (int i=0; i<z; i++)
        extra[i] = new int[x];
    ...
    {
        shiftplane(shift,0,planea,extra);
        if (checkplane(newplanea, extra)) {if (shift == 0) return 8; else if (shift == 2) return 2; else return 1;}
        shiftplane(shift,1,planea,extra);
        if (checkplane(newplanea, extra)) {if (shift == 0) return 4; else if (shift == 6) return 2; else return 1;}
    }
    return -1;
}
...
int main()
{
    ...
    //to find all possible cycle lengths, and its frequency
    int **cyclearray = new int*[500];
    for (int i=0;i<500;i++)
        cyclearray[i]=new int[2];
    
    int cycles = 0;
    
    //the cycle length is initialized to 0, its frequency is initilized to 1
    for (int i=0;i<500;i++)
    {
        cyclearray[i][0]=0;
        cyclearray[i][1]=0;
    ...
    for (int q=0; q<total_inputs; q++)
    {
        ...
        else
        {
            ...
            do
            {
                for (int shift = 0; shift < x; shift++)
                {
                    shiftcheck(shift,0,planea,planeb,planec,newplanea,newplaneb,newplanec);
                    parsed[binarytodecimal(statetobinary(newplanea,newplaneb,newplanec))] = 1;
                    shiftcheck(shift,1,planea,planeb,planec,newplanea,newplaneb,newplanec);
                    parsed[binarytodecimal(statetobinary(newplanea,newplaneb,newplanec))] = 1;
                }
            ...
            cout<<"Last state in cycle: "<<endl;
            printstatedecimal(check);
            
            
            cyclearray[cycles][0] = count;
            cyclearray[cycles][1] = offset;
    ...
    //final results after breaking out of the for loop
