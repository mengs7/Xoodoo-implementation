...
void mixinglayer(int* planea[2], int* planeb[2], int* planec[2], int* temp[2])
{
    ...
    shiftplane(1,2,E,P);//shifts E, puts new value into P
...
int whileconditioncheck(int* planea[2], int* newplanea[2])
{
    ...
    for (int shift = 0; shift < z; shift++)
    {
        ...
        if (checkplane(newplanea, extra)) {if (shift==0) return 8; else if (shift == 2) return 4; else return 2;}
        shiftplane(1,shift,planea,extra);
        if (checkplane(newplanea, extra)) {if ((shift==1) || (shift == 3)) return 2; else return 4;}
    }
    return -1;
}
        ...
        else
        {
            ...
            do
            {
                for (int shift = 0; shift < z; shift++)
                {
                    ...
                    shiftcheck(1,shift,planea,planeb,planec,newplanea,newplaneb,newplanec);
                    parsed[binarytodecimal(statetobinary(newplanea,newplaneb,newplanec))] = 1;
                }
