...
void mixinglayer(int* planea[1], int* planeb[1], int* planec[1], int* temp[1])
{
    ...
    shiftplane(0,0,P,temp); //shifts P, state_parsed in temp
    shiftplane(0,0,E,P);//shifts E, puts new value into P
...
int whileconditioncheck(int* planea[1], int* newplanea[1])
{
    ...
    for (int shift = 0; shift < z; shift++)
    {
        ...
        if (checkplane(newplanea, extra)) return 1;
    }
    return 0;
}
...
int main()
{
    ...
    int **cycle_lengths = new int*[50];
    for (int i=0;i<50;i++)
        cycle_lengths[i]=new int[2];
    ...
    for (int q=0; q<total_inputs; q++)
    {
        ...
        else
        {
            ...
            } while(!whileconditioncheck(state,check));
            ...
            cout<<"Total number of cycles up until now = "<<totalcycles<<endl<<endl;
            
            //check if the cycle length already exists in the array and if not, add it to the array and increase the frequency of the cycle length's occurance by 1
            int exists = 0;
            for (int i=0;i<50;i++)
            {
            ...
            if (exists==0)
            {
                cycle_lengths[cyclelocation][0] = count;
                cycle_lengths[cyclelocation][1]+=1;
                cyclelocation+=1;
            }
        }
    }
    ...
    cout<<"Possible cycles and their count: "<<endl;
    int total = 0;
    for (int i=0; i<50; i++)
    {
        if (cycle_lengths[i][0]!=0)
        {
            cout<<"Cycle length of "<<cycle_lengths[i][0]<<" has occured "<<cycle_lengths[i][1]<<" times."<<endl;
            total+=cycle_lengths[i][1];
        }
    }
    cout<<endl;
    cout<<"Total number of cycles: "<<total<<endl;
