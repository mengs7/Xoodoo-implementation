...
void mixinglayer(int* planea[1], int* planeb[1], int* planec[1], int* temp[1])
{
    ...
    shiftplane(0,7,P,temp); //shifts P, state_parsed in temp
...
int whileconditioncheck(int* planea[1], int* newplanea[1])
{
    ...
    for (int shift = 0; shift < z; shift++)
    {
        ...
        if (checkplane(newplanea, extra)) {if (shift == 0) return 8; else if (shift == 4) return 4; else if ((shift ==2) || (shift == 6)) return 2; else return 1;}
    }
    return -1;
}
